...
アプリ側は `src/utils/mesh-code.ts` で座標からコードを求め、`quake-mesh.json` の `cells` を引く。
市内7地点で API が返したコードと計算結果が一致すること、および生成時に全メッシュ（約1,000枚）で一致することを確かめている。
...
- `cells` はメッシュコードで引く辞書で、形状を持たない（矩形はコードから復元する）
...
- 生成物は 160KB 前後（整形前）。リポジトリには Prettier で整形した状態で入るため 270KB 前後になるが、バンドルには影響しない
