...
e-ZUKA スマートアプリコンテスト2026 の出品版を開発している(2026-08-12 応募、一次審査通過。最終審査 2026-10-10、プレゼン資料の提出 09-30、凍結 09-27)。

## 正とするドキュメント

この CLAUDE.md には要点だけを書く。詳細は以下が正であり、矛盾したらそちらに従う。
要件・仕様・調査・設計・提出物の文書は公開リポジトリに置かず、Obsidian の `~/workspace/Obsidian/projects/のおがたポケット/` に置く(2026-09-15 決定)。repo に残す文書は README、この CLAUDE.md、UI カンプだけ。

- Obsidian「直方ベース 要件定義書」:要件・スコープ・判断日・スケジュール。**機能の追加や削減はここの判断日ルールに従う。会話の流れで勝手にスコープを増やさない**
- Obsidian「直方ベース データソース仕様書」:全データソースの仕様(エンドポイント・フィールド・フィルタ・出典)。コード内のコメントでは「データソース仕様書 §N」と参照する
- Obsidian「直方ベース v1.6 設計書」「直方ベース v1.6 タスク分割」「直方ベース 調査 …」「直方ベース ProtoPedia 作品ページ」「直方ベース デモ動画台本」:設計、調査、提出物
- `docs/design/ui-mockup.html`:UI カンプ(画面 ID S-01〜S-10、配色・状態表現のトークン。S-01/S-08/S-09/S-10 の絵は v1.6 の操作 UI に未更新で、凍結後に実機のスクリーンショットで置き換える)
...
5. kawabou(river.go.jp)と九州電力送配電の停電情報へはリンクアウトのみ。スクレイピング・API 呼び出しを実装しない(どちらも利用規約で機械取得が禁止)
